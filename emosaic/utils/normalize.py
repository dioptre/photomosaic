...
def resize_image(target_height, target_width, img, scale=1):
    border_v = 0
    border_h = 0
    ...
    img = cv2.resize(img, (int(target_width/float(scale)), int(target_height/float(scale))))
    return img
            ...
            try:
                ...
                if extension == "heic":
                    try:
                        ...
                        cv2.imwrite(o, resize_image(target_height, target_width, np_array, scale))
                        continue
                    except:
                        image = cv2.imread(f)
                        cv2.imwrite(o, resize_image(target_height, target_width, image, scale))
